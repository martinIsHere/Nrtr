...
DrawingComponent::DrawingComponent(
    ...
    const int walkSpriteWidthNHeight, const int spriteSheetColumns,
    const int spriteSheetRows, const int amountOfWalkingAnimationFrames,
    const unsigned int fps,  // frames per state of animation
    const Camera* cam) {
  m_FPS = fps;

  // Load BMP into surface variable
  // SDL_Surface* spriteSheetSurface = SDL_LoadBMP(std::string("../res/imgs/" +
  // spriteTitle).c_str());

  SDL_Surface* spriteSheetSurface = SDL_LoadBMP(spriteTitle.c_str());
  // TODO
  // allow ./bin/exe finding ./res/... from ./bin
  if (!spriteSheetSurface) {
    log(("../" + spriteTitle).c_str());
    SDL_Surface* spriteSheetSurface =
        SDL_LoadBMP(("../" + spriteTitle).c_str());
  }

  if (!spriteSheetSurface) {
    std::cout << SDL_GetError();
    log("SDL_Surface* spriteSheetSurface = SDL_LoadBMP(spriteTitle.c_str()); "
        "failed");
    exit(-1);
  }
  ...
  m_destRect = new SDL_Rect{0, 0, AVERAGE_ENTITY_SIZE_PIXELS,
                            AVERAGE_ENTITY_SIZE_PIXELS};
...
const Camera* DrawingComponent::getCurrentlyUsedCamPtr() const { return m_cam; }
...
const void DrawingComponent::initCustomAnimation(
  ...
  ent->getComponent<PositionComponent>().set_isAbleToMove(
      isAbleToMoveDuringAnimation);
  customAnimationHasEnded_notifier = false;
}
...
void DrawingComponent::update_walkingAnimation() {
  if (m_posComp->isMoving()) {  // if moving, update the animation frame
    ...
    if (m_frameCounter >= m_framesPerImage) {  // if image has been shown for
                                               // correct amount of frames
      if (m_animationTick - 1 <
          m_animationFramesPerState -
              1) {  // increment animationtick or reset if at limit. circular
        m_animationTick += 1;
      } else {
        m_animationTick = 1;  // 1 because the spritesheet walking animation
                              // starts one over. idle is 0
      }
      m_frameCounter = 0;
    } else {
      m_frameCounter++;
...
void DrawingComponent::update_customAnimation() {
  if (currentRepetition <
      amountOfRepetitions) {  // if not moving, update the animation frame
    if (m_frameCounter >= m_framesPerImage) {  // if image has been shown for
                                               // correct amount of frames
      if (m_animationTick <
          amountOfFramesForCustomAnimation -
              1) {  // increment animationtick or reset if at limit. circular
        m_animationTick += 1;
      } else {
        m_animationTick = startX;
    ...
    m_animationTick = 0;
    m_frameCounter = 0;
    if (!isAbleToMoveDuringAnimation)
      ent->getComponent<PositionComponent>().set_isAbleToMove(true);
    customAnimationHasEnded_notifier = true;
  }
}

const bool DrawingComponent::is_in_viewable_area() const {
  if (m_posComp->getx() + MAX_SPRITE_SIZE >
          *m_cameraOffsetX  // m_cameraOffsetX being
                            // translateXcoordFromWinToMap(0)
      && m_posComp->getx() < *m_cameraOffsetX + (int)m_cam->getWinWidth()) {
    if (m_posComp->gety() + MAX_SPRITE_SIZE >
            *m_cameraOffsetY  // m_cameraOffsetY being
                              // translateYcoordFromWinToMap(0)
...
void DrawingComponent::draw() {
  ...
  // m_cameraOffsetx/y can be seen as values of a vector describing translation
  // from camera position to top-left corner of field of vision.
  //										vvvvvv
  // this will find the coordinates of the entity relative to the window based
  // on the position in the "game world"
  m_destRect->x = m_posComp->getx() - *m_cameraOffsetX;
  m_destRect->y = m_posComp->gety() - *m_cameraOffsetY;

  // Update src rect by animation tick
  if (m_animationTick > m_spriteSheet->nWidth)
    log("m_animationTick > m_spriteSheet->nWidth");
...
void DrawingComponent::draw_WalkingAnimationframe_according_to_direction() {
  ...
  switch (m_facingDir) {
    case DIR_LEFT:
      m_srcRect->y = 0;  // y coordinate corrosponding to spritesheet walking
                         // left animation
      SDL_RenderCopy(m_ren, m_spriteSheet->tex, m_srcRect, m_destRect);
      break;
    case DIR_RIGHT:
      m_srcRect->y = 0;  // ---||---
      SDL_RenderCopyEx(m_ren, m_spriteSheet->tex, m_srcRect, m_destRect, 0, 0,
                       SDL_FLIP_HORIZONTAL);
      break;
    case DIR_UP:
