...
PositionComponent::PositionComponent(int x, int y) : m_X(x), m_Y(y) {
  ...
  default_acceleration = float(
      1.6 * (double)TILE_SIZE_PIXELS /
      (double)
          targetFPS);  // DEFAULT ACCELERATION   // 1.4 if TILE_SIZE_PIXELS = 64
  default_natural_deceleration = float(
      0.5 * (double)TILE_SIZE_PIXELS /
      (double)targetFPS);  // 0.8 if TILE_SIZE_PIXELS = 64 and targetFPS = 60
  natural_deceleration = default_natural_deceleration;
  max_vel = float(
      8 * (double)TILE_SIZE_PIXELS /
      (double)targetFPS);  // 10 if TILE_SIZE_PIXELS = 64 and targetFPS = 60
  isAbleToMove = true;
  isFrictionless = false;
}

PositionComponent::PositionComponent(int x, int y, uint32_t blockSize)
    : m_X(x), m_Y(y), blockSize(blockSize) {
  m_velocity_vector = nullptr;
  m_acceleration_vector = nullptr;
...
int& PositionComponent::getx() { return m_X; }
int& PositionComponent::gety() { return m_Y; }

float PositionComponent::getMaxVel() const { return max_vel; }

void PositionComponent::apply_natural_deceleration() {
  // check if abs of velocity x is less than NATURAL_DECELERATION
  // if it is, set x-velocity to zero if not do nothing
...
void PositionComponent::setPos_tileCoords(const int x_inTiles,
                                          const int y_inTiles) {
  if (blockSize == 0) {
...
std::array<bool, 4>& PositionComponent::getDir() { return m_Directions; }

uint32_t& PositionComponent::getFacingDir() { return m_facingDir; }

void PositionComponent::setFacingDir(uint32_t dir) { m_facingDir = dir; }
...
float& PositionComponent::getVelx() { return m_velocity_vector->x; }

float& PositionComponent::getVely() { return m_velocity_vector->y; }

Vector* PositionComponent::getVel_vector() { return m_velocity_vector; }
...
void PositionComponent::set_isAbleToMove(bool a) { isAbleToMove = a; }

void PositionComponent::set_isFrictionless(bool a) { isFrictionless = a; }
