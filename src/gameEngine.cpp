...
void set_pixel(const SDL_Surface* surface, const uint64_t x, const uint64_t y,
  ...
  Uint8* target_pixel =
      (Uint8*)surface->pixels  // address of first pixel
      + y * surface->pitch     // add y * amountOfPixelsPerRowOfPixels
      + x * surface->format
                ->BytesPerPixel;  // add x * sizeOfOnePixelInMemory (each pixel
                                  // is represented by 32 bits)
...
void makeSurfaceTransparent(SDL_Surface* surface) {
  ...
  for (uint32_t i = 3; i < sizeOfPixelArray_inBytes; i += pixelSize_inBytes) {
...
void clearWithDitherEffect(SDL_Surface* surface) {
  ...
  uint32_t oldIsOdd = -1;
  uint32_t isOdd = 0;
  for (uint32_t i = 0; i < sizeOfPixelArray_inBytes - pixelSize_inBytes * 4;
...
GameEngine::GameEngine(const uint32_t nWidth, const uint32_t nHeight,
  ...
  win = SDL_CreateWindow(title.c_str(), SDL_WINDOWPOS_CENTERED,
                         SDL_WINDOWPOS_CENTERED, nWinWidth, nWinHeight, 0);
  if (win == 0) {
    std::cout << "Failed to create window\n";
    bRunning = false;
    exit(1);
  ...
  if (gMusic == NULL) gMusic = Mix_LoadMUS("../res/audio/stanczyk.wav");
  if (gMusic == NULL) {
  ...
  theaterEngine->init(m_stateManagerPtr, &m_entityManager, currentMapPtr,
                      renPtr, &nWinWidth, &nWinHeight);
  ...
  SDL_Surface* vignetteSurface =
      SDL_CreateRGBSurface(0, nWinWidth, nWinHeight, 32, 0xFF000000, 0x00FF0000,
  ...
  while (SDL_PollEvent(Event)) {
    switch (Event->type) {
      ...
      case SDL_MOUSEBUTTONDOWN:
        ...
        std::cout
            << "x: "
            << theaterEngine->getCurrentMap()->getCellx_fromCoord(
                   mousePos[0] +
                   *theaterEngine->getCurrentMap()->getCam()->getOffsetXPtr())
            << "\n";
        std::cout
            << "y: "
            << theaterEngine->getCurrentMap()->getCelly_fromCoord(
                   mousePos[1] +
                   *theaterEngine->getCurrentMap()->getCam()->getOffsetYPtr())
            << "\n";
        log("--------------");
        a++;
        break;
      ...
      case SDL_KEYUP:
        switch (Event->key.keysym.sym) {
          ...
          case SDLK_d:
            ...
            break;
          case SDLK_t:
            (*currentMapPtr)->get_drawingSolidStates_bool() =
                (*currentMapPtr)->get_drawingSolidStates_bool() ? false : true;
            break;
          case SDLK_l:
          ...
          case SDLK_SPACE:
            ...
            break;
          case SDLK_1:
            playerEntity->getComponent<DrawingComponent>().initCustomAnimation(
                4, 0, 6, 1, false);
            break;
        }
        break;
...
const void GameEngine::test_portalAnimationFunction() {
  if (playerEntity->getComponent<DrawingComponent>()
          .customAnimationHasEnded() &&
      playerEntity->getComponent<DrawingComponent>()
              .getPrevCustomAnimationY() == 4) {
    playerEntity->getComponent<PositionComponent>().moveForward(
        2 * TILE_SIZE_PIXELS);
  }
}
...
void GameEngine::update() {
  ...
  } else if (m_stateManagerPtr->get() ==
             m_stateManagerPtr->state_blockTransition) {
    (*currentMapPtr)->update();
...
void GameEngine::sortEntityArray() {
  ...
  // sort the arrayOfActiveEntities going highest y-value to lowest y-value
  // !!!!! TODO
  while (arrayOfActiveEntitiesPtr->size() > 0) {
    ...
    if (arrayOfActiveEntitiesPtr->size() != 1) {
      for (int i = 1; i < arrayOfActiveEntitiesPtr->size(); i++) {
        if (arrayOfActiveEntitiesPtr->at(i)
                ->hasComponent<PositionComponent>()) {
          if (arrayOfActiveEntitiesPtr->at(i)
                  ->getComponent<PositionComponent>()
                  .gety() < arrayOfActiveEntitiesPtr->at(suspectedIndex)
                                ->getComponent<PositionComponent>()
                                .gety()) {
            suspectedIndex = i;
          }
    ...
    bufferArrayOfEntities.push_back(
        arrayOfActiveEntitiesPtr->at(suspectedIndex));
    arrayOfActiveEntitiesPtr->erase(arrayOfActiveEntitiesPtr->begin() +
                                    suspectedIndex);
  }
...
void GameEngine::draw(bool isPresenting) {
  ...
  // temporary
  SDL_RenderCopy(renPtr, pixelTexturePtr, NULL,
                 new SDL_Rect{0, 0, (int)nWinWidth, (int)nWinHeight});
...
// clean up later
void GameEngine::transitionDraw_boxes(const uint32_t&& r, const uint32_t&& g,
                                      const uint32_t&& b) {
  if (elapsedFrames == 0) {
    boxWidth =
        int(nWinWidth /
            amountOfBoxesX);  // 20 -> amount of boxes to be drawn across window
  ...
  if (boxCount < (amountOfBoxesX + 1) * amountOfBoxesY) {
    ...
    drawRect.x = (boxCount % (amountOfBoxesX + 1)) * boxWidth;
    drawRect.y = int(boxCount / (amountOfBoxesX + 1)) * boxHeight;
    for (int i = boxCount - (amountOfBoxesX + 1) * amountOfBoxesY;
         i < (amountOfBoxesX + 1) * amountOfBoxesY; i++) {
        ...
        if (in_currentMapPtr != nullptr) {
                for (Entity* entity : *arrayOfActiveEntitiesPtr) {
                        ...
                        if (entity->hasComponent<PositionComponent>()) {
                                entity->getComponent<PositionComponent>().setPos(TILE_SIZE_PIXELS*2,
TILE_SIZE_PIXELS*3);
                        }
                }
...
bool GameEngine::alive() { return bRunning; }
