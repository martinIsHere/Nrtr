...
GameMap::GameMap(SDL_Renderer* ren, const std::string map,
                 const std::string spriteSheetPath, const int spriteSheetTilesX,
                 const int spriteSheetTilesY, const int windowWidth,
                 const int windowHeight, int* cameraPosx, int* cameraPosy,
                 bool smallMap) {
  m_ren = ren;
  ...
  // load spriteSheet
  m_spriteSheet0 =
      new SpriteSheet{nullptr, 16, spriteSheetTilesX, spriteSheetTilesX};

  m_spriteSheet0->tex =
      SDL_CreateTextureFromSurface(m_ren, SDL_LoadBMP(spriteSheetPath.c_str()));
  if (m_spriteSheet0->tex == nullptr) {
    m_spriteSheet0->tex = SDL_CreateTextureFromSurface(
        m_ren, SDL_LoadBMP(("../" + spriteSheetPath).c_str()));
  }
  if (m_spriteSheet0->tex == nullptr) {
    m_spriteSheet0->tex = SDL_CreateTextureFromSurface(
        m_ren, SDL_LoadBMP(("../" + spriteSheetPath).c_str()));
  ...
  m_mainCamera =
      new Camera(cameraPosx, cameraPosy,         // position
...
const std::string GameMap::getLoadedMapPath() const { return mapPath; }

void GameMap::update() { m_mainCamera->update(); }
...
const int& GameMap::getMapWidth_tiles() { return m_mapWidth; }

const int& GameMap::getMapHeight_tiles() { return m_mapHeight; }
    ...
    for (int x = initialX; x < endBoundaryX; x++) {
      *m_tempDstRect = {int((x * m_blockSize) - offsetX),
      ...
      if (id != 0) {
        ...
        *m_tempSrcRect = {m_xId * m_spriteSheet0->nSize,
                          m_yId * m_spriteSheet0->nSize, m_spriteSheet0->nSize,
                          m_spriteSheet0->nSize};

        SDL_RenderCopyEx(m_ren, m_spriteSheet0->tex, m_tempSrcRect,
                         m_tempDstRect, 0, 0,
...
void GameMap::drawSecondLayer() {
  ...
  dstRect2->x =
      ...
      offsetX;
  dstRect2->y =
      (mousePositionY >= 0 ? int(mousePositionY / TILE_SIZE_PIXELS)
                           : int(mousePositionY / TILE_SIZE_PIXELS) - 1) *
          TILE_SIZE_PIXELS -
      offsetY;
...
int& GameMap::getBlockSize() { return m_blockSize; }

Camera* GameMap::getCam() { return m_mainCamera; }

bool& GameMap::get_drawingSolidStates_bool() { return m_isDrawingSolidStates; }
...
const int GameMap::getCellx_fromCoord(const int x) const {
  if (x >= 0) return int(x / m_blockSize);
  if (x < 0) return int(x / m_blockSize) - 1;
}

const int GameMap::getCelly_fromCoord(const int y) const {
  if (y >= 0) return int(y / m_blockSize);
  if (y < 0) return int(y / m_blockSize) - 1;
}
